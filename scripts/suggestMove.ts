...
const isSuitOpposite = (compare: Suit, to: Suit) => {
	switch (compare) {
		case 'Diamond':
		case 'Heart':
			return to === 'Club' || to === 'Spade';
		case 'Club':
		case 'Spade':
			return to === 'Diamond' || to === 'Heart';
	}
};
...
export class PlayPile extends Pile {
	public canAdd = (card: Card) =>
...
export type PileType = 'draw' | 'foundation' | 'play';

export interface MovePile {
	pile: PileType;
	index: number;
}
...
export class Game {
	...
	constructor(
		_drawPile?: DrawPile,
		_foundationPiles?: FoundationPile[],
		_playPiles?: PlayPile[]
	) {
		if (_drawPile && _foundationPiles && _playPiles) {
	...
	public suggestMove(): Move | null {
		...
		if (
			this.drawPile.top?.value === 'Ace' ||
			this.drawPile.top?.value === 2
		) {
		...
		for (const [
			...
			if (foundationPile.top) {
				...
				if (
					this.drawPile.top &&
					foundationPile.canAdd(this.drawPile.top)
				) {
					return {
						from: {
							pile: 'draw',
